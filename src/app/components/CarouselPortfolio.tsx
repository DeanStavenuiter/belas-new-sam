  ...
  return (
    <>
      <div className='hidden sm:block'>
        <div className={`flex w-[755px] flex-col`}>
          <div
          ...
          >
            {cards.map((card, index) => (
              <div
                key={index}
                className={` ${index === currentIndex ? 'active' : ''}  ${index !== cards.length - 1 ? 'pr-[35px]' : ''}`}
              >
                {card}
              </div>
            ))}
      ...
      <div className='mt-[40px] flex-row justify-center gap-[9px] md:justify-start md:mt-[25px] hidden sm:flex'>
        <div
    ...
    <div
      className='carousel-container overflow-y-hidden overflow-x-scroll sm:hidden'
      onScroll={handleScroll}
      ref={cardsContainerRef}
    >
      <div
